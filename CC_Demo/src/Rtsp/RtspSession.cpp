...
//track id
	...
	case PlayerBase::RTP_UDP: {
		...
		/*将客户端的地址存储起来，里面包含IP和客户端的端口号*/
		TraceL<<"[merlin]:"<<"serverAddr="<<"socket:"<<(pSock->rawFD())<<" port:"<<iSrvPort<<endl;
		TraceL<<"[merlin:]:"<<"trackIdx="<<trackIdx<<" clientAddr="<<(getPeerIp())<<":"<<(ui16PeerPort)<<endl;
...
bool RtspSession::handleReq_Play() {
	TraceL<<"[merlin]"<<"m_uiTrackCnt="<<m_uiTrackCnt<<" line:"<<__LINE__<<endl;
	if (m_uiTrackCnt == 0) {
		//还没有Describe
		return false;
	}
...
inline bool RtspSession::findStream() {
	...
	/*比如url是：rtsp://192.168.5.90/record/11-09-38.mp4*/
    string strHost = FindField(m_strUrl.data(), "://", "/"); /*获取rtsp服务器端的IP地址:192.168.5.90*/
    m_strApp = FindField(m_strUrl.data(), (strHost + "/").data(), "/"); /*视频文件所在的目录名:record*/
    m_strStream = FindField(m_strUrl.data(), (strHost + "/" + m_strApp + "/").data(), NULL); /*视屏文件的名称:11-09-38.mp4*/

	//std::cout<<"["<<__func__<<"]:"<<"strHost="<<strHost<<" m_strApp="<<m_strApp<<" m_strStream="<<m_strStream<<std::endl;
...
/***************************************************************************
* Description:发送rtp数据包
* InParam pkt:需要发送的RTP数据包
*****************************************************************************/
inline void RtspSession::sendRtpPacket(const RtpPacket& pkt) {
	//InfoL<<(int)pkt.Interleaved;
	//TraceL<<"[merlin]:"<<"m_rtpType="<<m_rtpType<<" pkg.len="<<(pkt.length)<<" line:"<<__LINE__<<endl;
	switch (m_rtpType) {
	case PlayerBase::RTP_TCP: {
	...
	case PlayerBase::RTP_UDP: {
		...
		auto peerAddr = m_apPeerUdpAddr[iTrackIndex]; /*RTP客户端的地址*/
		if (!peerAddr) {
			return;
		}
		pSock->sendTo((char *) pkt.payload + 4, pkt.length - 4, peerAddr.get());  /*发送RTP数据包*/
	}
		break;
	default:
		break;
